...
- Компилировать `vo` / `bed` как смешанный звук, `captions` как sidecar и
  `brand` как overlay на concat картинки
- Назвать join `fade` на score, не переписывая соседние slot_encode
...
- Предпросматривать выбранный scion через decode и composite, с синхронным
  звуком и overlay mix
- Синхронизировать blob в корень object-store с поиском недостающих
  blob. Удалённого compile нет.
