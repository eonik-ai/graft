...
- decode와 composite로 선택한 scion을 미리 본다
- 누락 blob 발견과 함께 object-store 루트로 blob을 동기화한다

## 하지 않는 일

graft는 speed/retime을 적용하지 않고, 임의의 mid-GOP 소스를 수리하지 않으며,
NLE의 effects, grades, generators를 왕복하지 않는다. 픽셀 위의 Git도, Git
이력의 대체재도, 잠금 서버도, DAM도, 리뷰 도구도 아니다.
