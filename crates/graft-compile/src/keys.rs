...
use graft_score::{Binding, Dest, Scion, Slot};
...
pub fn scion_hash(scion: &Scion, slot_keys: &[ActionKey], kerf_keys: &[ActionKey]) -> ActionKey {
...
mod tests {
    ...
    fn hook_and_body_keys_differ() {
        ...
        let hk = slot_encode_key(hook, &scion.bindings["hook"], &scion.dest);
        let bk = slot_encode_key(body, &scion.bindings["body"], &scion.dest);
        assert_ne!(hk, bk);
